...
export default function EquiposLista() {
  ...
  const equiposFiltrados = (() => {
    // Primero filtrar por programa adicional si está seleccionado
    let filtrados = programaAdicionalSeleccionado.length > 0
      ? equipos.filter(e => programaAdicionalSeleccionado.every(p => e.programa_adicional_ids?.includes(Number(p))))
      : equipos;

    // Luego filtrar por tipo de estado
  ...
  // Calcular estadísticas para PanelControl (sin filtrar por estado)
  const equiposStats = programaAdicionalSeleccionado.length > 0
    ? equipos.filter(e => programaAdicionalSeleccionado.every(p => e.programa_adicional_ids?.includes(Number(p))))
    : equipos;

  const stats = {
    total: equiposStats.length,
    active: equiposStats.filter(e => e.estado === 'Activo').length,
    maintenance: equiposStats.filter(e => e.estado === 'Mantenimiento').length,
    inactive: equiposStats.filter(e => e.estado === 'Inactivo').length,
  };
